...
export default function TalleresPage() {
  ...
  hatha:{title:"Hatha Yoga",desc:"Yoga para dar paso a un post taller en el bar, compartiendo unas cervezas."},
  acuarelas:{title:"Acuarelas",desc:"El segundo taller a cargo de Lucía..."},
  circulo:{title:"Círculo de mujeres",desc:"Espacios íntimos para la escucha..."},
  yoga_facial:{title:"Yoga Facial",desc:"Practicamos Yoga Facial..."},
  dibujar:{title:"Dibujar para encontrarse",desc:"Un espacio para explorar..."}
};
const tx = (k:string, fb:string) => {
  const v = t(k) as string | undefined;
  return v && typeof v === "string" ? v : fb;
};
const title = (id:string) => tx(`workshops.items.${id}.title`, fallbacks[id].title);
const desc  = (id:string) => tx(`workshops.items.${id}.desc`,  fallbacks[id].desc);
...
<section className={styles.section}>
  <div className={styles.container}>
    <h2 className={styles.sectionTitle}>{t("workshops.upcoming") as string}</h2>

    {upcoming.length === 0 ? (
      <div className={styles.empty}>
        <h3 className={styles.emptyTitle}>{t("workshops.empty.title") as string}</h3>
        <p className={styles.emptyDesc}>{t("workshops.empty.desc") as string}</p>
      </div>
    ) : (
      <>
        {/* Featured */}
        <article className={styles.featured}>
          ...
          <div className={styles.featuredBody}>
            <h3 className={styles.featuredTitle}>{title(featured!.id)}</h3>
            <small className={styles.date}>{fmt(featured!.dateISO)}</small>
      ...
      <section className={styles.section}>
        <div className={styles.container}>
          <h2 className={styles.sectionTitle}>{t("workshops.past") as string}</h2>
          <div className={styles.grid}>
            {past.map((w) => (
              <article key={w.id} className={styles.card}>
                <div className={styles.thumb}>
